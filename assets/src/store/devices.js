...
export default {
  ...
  getters: {
    ...
    findOldRegistered(state) {
      let index = state.devices.findIndex(currentDevice => (typeof currentDevice.id === 'string' || currentDevice.id instanceof String) && currentDevice.id.startsWith('device_'));
  ...
  mutations: {
    ...
    [CREATING_DEVICE_SUCCESS](state, data) {
      ...
      let index = state.devices.findIndex(currentDevice => currentDevice.id == previousId);

      if (0 <= index) {
        state.devices[index].id = device.id;
        ...
        state.devices[index].deviceXDeviceGroups = device.deviceXDeviceGroups;
    ...
    [UPDATE_DEVICE_SUCCESS](state, data) {
      let device = data.device;
      let previousId = data.origId;
    ...
    [UNREGISTER_DEVICE](state, device) {
      let index = state.devices.findIndex(currentDevice => (currentDevice.id === device.id));

      return state.devices.splice(index, 1);
    }
  },
  actions: {
    async create({ commit }, device) {
      ...
      try {
        const origId = device.id; // expect the current id is autogenerated by frontend and leads in problems during save process. but need to persist to replace current entry with response
        device.id = null;
        let response = await DeviceController.create(device);
        commit(CREATING_DEVICE_SUCCESS, {device: response.data, origId: origId});
    ...
    async update({ commit }, device) {
      ...
      try {
        let response = await DeviceController.update(device);
        commit(UPDATE_DEVICE_SUCCESS, {device: response.data, origId: device.id});
        return response.data;
      } catch (error) {
    ...
    async register({ commit }, device) {
      return commit(REGISTER_DEVICE, device);
    },
    async unregister({ commit }, device) {
      return commit(UNREGISTER_DEVICE, device);
    }
  }
};
